...
function countMainCourseItems(menu) {
  ...
  menu.forEach((menuObjects) => {
    if (menuObjects.category == "Main Course") {
      item = item + 1;
    }
  });

  // filter
  let mainCourse = menu.filter((menuItem) => {
...
function getCheapestMenuItem(menu) {
  // Your code here
  return menu.reduce((cheapestItem, currentItem) =>
    currentItem.price < cheapestItem.price ? currentItem : cheapestItem
  );
}
